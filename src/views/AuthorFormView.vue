...
async function loadAuthor() {
  ...
  try {
    ...
    loadError.value = reason instanceof ApiError && reason.status === 404 ? 'Автор не найден.' : 'Не удалось загрузить форму. Попробуйте ещё раз.'
  } finally {
    loading.value = false
  }
}
...
<template>
  <section class="form-page">
    ...
    <div v-else-if="loadError" class="catalog-status catalog-status--error" role="alert"><p>{{ loadError }}</p><button class="button button--quiet" type="button" @click="loadAuthor">Повторить</button></div>
    <form v-else class="author-form" @submit.prevent="submit">
      ...
      <div class="form-actions"><button class="button" :disabled="submitting" :aria-busy="submitting">{{ submitting ? 'Сохраняем…' : 'Сохранить' }}</button><RouterLink class="button button--quiet" :to="{ name: 'authors' }">Отмена</RouterLink></div>
    </form>
  </section>
</template>
