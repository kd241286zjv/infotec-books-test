...
function readSession(): Session | null {
  try { return JSON.parse(sessionStorage.getItem(storageKey) ?? 'null') as Session | null } catch { return null }
}

const session = ref<Session | null>(readSession())

function save(response: LoginResponse) {
  session.value = { token: response.token, expiresAt: response.expires_at, user: response.user }
